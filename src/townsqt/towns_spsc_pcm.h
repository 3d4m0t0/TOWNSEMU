...
// Producer: VM thread only. Consumer: miniaudio callback only.

class TownsSpscPcmRing
{
public:
	...
	// Producer only.
	size_t Push(const int16_t *stereo_frames,size_t frames)
	{
		if(nullptr==stereo_frames || 0==frames || 0==cap_)
		{
			return 0;
		}
		const uint64_t h=head_.load(std::memory_order_acquire);
	...
	// Consumer only.
	size_t Pop(int16_t *stereo_frames,size_t frames)
	{
		if(nullptr==stereo_frames || 0==frames || 0==cap_)
		{
			return 0;
